...
mongoose.connect(
  "mongodb+srv://benedictusdito:kebonjeruk21@bencluster-o0agg.mongodb.net/bendatabasejc9?retryWrites=true&w=majority",
  {
    // Parser string URL
...
app.delete("/users/:id", (req, res) => {
  ...
  User.findByIdAndDelete(data_id).then(user => {
    if (!user) {
      return res.send("User tidak di temukan");
    }
...
app.delete("/tasks", (req, res) => {
  ...
  // Menghapus satu task
  Task.findByIdAndDelete({ _id: data_taskid }).then(result => {
    res.send(result);
  });
